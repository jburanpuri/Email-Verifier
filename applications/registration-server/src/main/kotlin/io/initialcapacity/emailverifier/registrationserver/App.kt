...
fun main(): Unit = runBlocking {
    ...
    val registrationRequestExchangeName = System.getenv("REGISTRATION_REQUEST_EXCHANGE_NAME") ?: "registration-request-exchange"
    val registrationRequestQueueName = System.getenv("REGISTRATION_REQUEST_QUEUE_NAME") ?: "registration-request"
    val routingKey = System.getenv("ROUTING_KEY") ?: "42"

    val registrationRequestExchange = RabbitExchange(
        name = registrationRequestExchangeName,
        type = "x-consistent-hash",
        routingKeyGenerator = { message: String -> message.hashCode().toString() }
    )

    connectionFactory.declareAndBind(exchange = registrationRequestExchange, queue = RabbitQueue(registrationRequestQueueName), routingKey = routingKey)

    listenForRegistrationRequests(
        connectionFactory,
        registrationRequestGateway,
        registrationNotificationExchange,
        RabbitQueue(registrationRequestQueueName)
    )
    registrationServer(
        port,
        registrationRequestGateway,
